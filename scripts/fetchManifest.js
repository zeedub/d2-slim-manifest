...
async function main() {
    try {
        ...
        console.log("Fetching item definitions...");
        ...
        // 4️⃣ Fetch DestinyPlugSetDefinition (THIS WAS MISSING!)
        console.log("Fetching plug set definitions...");
        const plugSetDefsRes = await fetch(`https://www.bungie.net${paths.DestinyPlugSetDefinition}`, {
            headers: { "X-API-Key": API_KEY },
        });
        const plugSetDefs = await plugSetDefsRes.json();

        // 5️⃣ Filter and slim weapons
        ...
        weaponsSlim.forEach(weapon => {
            weapon.sockets?.forEach(socket => {
                ...
                // Reusable plug items directly on socket
                socket.reusablePlugItems?.forEach(plug => {
                    plugHashes.add(plug.plugItemHash);
                });

                // FIXED: Get plugs from reusablePlugSetHash
                if (socket.reusablePlugSetHash) {
                    const plugSet = plugSetDefs[socket.reusablePlugSetHash];
                    if (plugSet?.reusablePlugItems) {
                        plugSet.reusablePlugItems.forEach(plug => {
                            plugHashes.add(plug.plugItemHash);
                        });
                    }
                }

                // FIXED: Get plugs from randomizedPlugSetHash
                if (socket.randomizedPlugSetHash) {
                    const plugSet = plugSetDefs[socket.randomizedPlugSetHash];
                    if (plugSet?.reusablePlugItems) {
                        plugSet.reusablePlugItems.forEach(plug => {
                            plugHashes.add(plug.plugItemHash);
                        });
        ...
        // 8️⃣ Build plugs.json with only used plugs
        const plugs = {};
        ...
        // Debug: Log some plug set info
        const samplePlugSet = Object.values(plugSetDefs)[0];
        console.log("Sample plug set structure:", {
            hash: samplePlugSet?.hash,
            hasReusablePlugItems: !!samplePlugSet?.reusablePlugItems,
            plugCount: samplePlugSet?.reusablePlugItems?.length || 0
        });
